...
Rmean = 3.0
sigma = 0.5
N = math.ceil(L/(2.0*(Rmean+3.5*sigma)))
DSIGMA = 13/1000
SIGMA = 72/1000
T = 293
##############################################################################
...
###################### работа по вытеканию #######################################    
    def work(self,p):
        SM = np.sum([self.area_meniscus_s(spheres[elm]) for elm in self.neighbors if not spheres[elm].mark] ) #пустые мениски
        SMM = np.sum([self.area_meniscus_s(spheres[elm]) for ind, elm in enumerate(self.neighbors) ] ) # все мениски
        SMZ = np.sum([ self.area_meniscus_o(spheres[elm]) for elm in self.neighbors if spheres[elm].mark ] ) #заполненные мениски
...
###################### работа по заполнению #######################################             
    def work_f(self,p):
        SM = np.sum([self.area_meniscus_s(spheres[elm]) for elm in self.neighbors if not spheres[elm].mark] ) 
        SMM = np.sum([self.area_meniscus_s(spheres[elm]) for ind, elm in enumerate(self.neighbors)] ) 
        SMZ = np.sum([ self.area_meniscus_o(spheres[elm]) for elm in self.neighbors if spheres[elm].mark ] )
        work = ( - p * 4.0 * 10**(-9) * math.pi * self.rad * self.rad * self.rad / 3.0 
            + (4.0 * math.pi * self.rad * self.rad - SMM) * DSIGMA 
            + SIGMA * (SM - SMZ) )
        if work < 0.0:
